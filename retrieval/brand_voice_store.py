...
class BrandVoiceStore:
    ...
    def _get_embedder(self):
        """Typed accessor:  ensure we have a loaded embedder or raises, rather than
        letting callers work with an Optional value the type checker can't narrow."""
        if self._embedder is None:
            raise RuntimeError("Embedder not loaded — call build() or retrieve() first.")
    ...
    def retrieve(self, query: str, top_k: int | None = None) -> List[VoiceChunk]:
        ...
        query_vec = self._get_embedder().encode([query], convert_to_numpy=True, normalize_embeddings=True)
        scores, idxs = self._get_index().search(query_vec.astype(np.float32), recall_k)
